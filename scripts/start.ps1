...
$hub = Start-Process powershell.exe -WindowStyle Hidden -PassThru -ArgumentList @(
  "-NoProfile","-ExecutionPolicy","Bypass","-Command",
  "cd '$Root\hub'; `$env:PORT='3800'; `$env:HUB_API_KEY='orion-proxy-key'; `$env:QWENPROXY_URL='http://localhost:3802'; `$env:QWENPROXY_KEY='orion-proxy-key'; npm run start *> '$LogDir\hub.log'"
)
