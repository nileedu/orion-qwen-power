...
sleep 3
(cd "$ROOT/hub" && PORT=3800 HUB_API_KEY=orion-proxy-key QWENPROXY_URL=http://localhost:3802 QWENPROXY_KEY=orion-proxy-key nohup npm run start > "$LOG_DIR/hub.log" 2>&1 & echo $! > "$LOG_DIR/hub.pid")
